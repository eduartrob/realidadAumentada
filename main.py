...
# 2.3. Objeto 3D (Cubo simple) para el Marcador 2
# Definimos los vértices de un cubo simple en coordenadas 3D relativas al origen del marcador.
# Cada fila es un vértice (x, y, z).
# El tamaño del cubo se escala para que sea visible sobre el marcador.
# Asumimos que el marcador está en el plano XY y Z es la profundidad.
cube_points = np.array([
    [-0.5, -0.5, 0],  # 0: Base inferior izquierda
    [ 0.5, -0.5, 0],  # 1: Base inferior derecha
    [ 0.5,  0.5, 0],  # 2: Base superior derecha
    [-0.5,  0.5, 0],  # 3: Base superior izquierda
    [-0.5, -0.5, 1],  # 4: Tapa inferior izquierda
    [ 0.5, -0.5, 1],  # 5: Tapa inferior derecha
    [ 0.5,  0.5, 1],  # 6: Tapa superior derecha
    [-0.5,  0.5, 1]   # 7: Tapa superior izquierda
], dtype=np.float32).reshape(-1, 3) * 0.1 # Escala el cubo para que sea más pequeño
...
def draw_3d_object(frame, rvec, tvec, camera_matrix, dist_coeffs, object_points):
    """
    Dibuja un objeto 3D simple (cubo) proyectando sus puntos 3D sobre el frame 2D.
    """
    ...
    # Dibujar las caras del cubo (líneas que conectan los vértices)
    # Base inferior
    cv2.line(frame, tuple(imgpts[0]), tuple(imgpts[1]), (0, 255, 0), 2)
    cv2.line(frame, tuple(imgpts[1]), tuple(imgpts[2]), (0, 255, 0), 2)
    cv2.line(frame, tuple(imgpts[2]), tuple(imgpts[3]), (0, 255, 0), 2)
    cv2.line(frame, tuple(imgpts[3]), tuple(imgpts[0]), (0, 255, 0), 2)

    # Tapa superior
    cv2.line(frame, tuple(imgpts[4]), tuple(imgpts[5]), (0, 255, 0), 2)
    cv2.line(frame, tuple(imgpts[5]), tuple(imgpts[6]), (0, 255, 0), 2)
    cv2.line(frame, tuple(imgpts[6]), tuple(imgpts[7]), (0, 255, 0), 2)
    cv2.line(frame, tuple(imgpts[7]), tuple(imgpts[4]), (0, 255, 0), 2)

    # Conectores verticales
    for i in range(4):
        cv2.line(frame, tuple(imgpts[i]), tuple(imgpts[i+4]), (0, 255, 0), 2)
            ...
            for i in range(len(ids)):
                ...
                if success:
                    ...
                    if marker_id == 0:
                        ...
                        # Superponer objeto 3D (cubo)
                        frame = draw_3d_object(frame, rvec, tvec, camera_matrix, dist_coeffs, cube_points)
                    else:
